...
class EmailService {
    ...
    async sendOverdueReminder(to, reminderData) {
        try {
            ...
            const emailTemplatePath = path.join(
                __dirname,
                '../views/emails/overdueReminder.ejs'
            );
            ...
            const subject = daysUntilDue > 2 
                ? `Reminder: Book Due in ${daysUntilDue} Days - ${this.companyName}`
                : `Urgent: Book Due in ${daysUntilDue} Days - ${this.companyName}`;
    ...
    async sendBookOverdueNotice(to, noticeData) {
        try {
            ...
            const emailTemplatePath = path.join(
                __dirname,
                '../views/emails/bookOverdueNotice.ejs'
            );
