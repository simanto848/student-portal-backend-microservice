...
export const errorHandler = (err, req, res, next) => {
    ...
    if (err instanceof z.ZodError) {
        return ApiResponse.validationError(res, 'Validation failed', buildValidationErrors(err.issues));
    }

    if (err?.name === 'ValidationError') {
        return ApiResponse.validationError(res, 'Validation failed', formatMongooseValidation(err));
    }
