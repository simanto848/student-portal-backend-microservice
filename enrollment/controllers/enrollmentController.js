...
class EnrollmentController {
  ...
  async getStudentSemesterEnrollments(req, res, next) {
    try {
      ...
      const enrollments = await enrollmentService.getStudentSemesterEnrollments(
        studentId,
        parseInt(semester)
      );
      return ApiResponse.success(res, enrollments);
  ...
  async completeBatchSemester(req, res, next) {
    try {
      ...
      const result = await enrollmentService.completeBatchSemester(
        batchId,
        parseInt(semester)
      );
      return ApiResponse.success(
        res,
        result,
        "Batch semester completed successfully"
      );
    } catch (error) {
      next(error);
    }
  ...
  async progressBatchSemester(req, res, next) {
    try {
      ...
      const result = await enrollmentService.progressBatchToNextSemester(
        batchId
      );
      return ApiResponse.success(
        res,
        result,
        "Batch semester progressed successfully"
      );
    } catch (error) {
      next(error);
    }
  ...
  async getBatchSemesterCourses(req, res, next) {
    try {
      ...
      const courses = await enrollmentService.getBatchSemesterCourses(
        batchId,
        parseInt(semester)
      );
