...
// Father info schema
const fatherInfoSchema = z.object({
...
export const studentProfileCreateValidation = (data) => {
    const schema = z.object({
        ...
        }).trim().min(1, { message: 'Student ID cannot be empty' }),
        
        // General Information
        shift: z.enum(['Morning', 'Day', 'Evening', 'Night', '']).optional(),
        group: z.string().trim().optional(),
        admissionFormSl: z.string().trim().optional(),
        admissionSeason: z.enum(['Spring', 'Summer', 'Fall', 'Winter', '']).optional(),
        admittedBy: z.string().trim().optional(),
        
        // Personal Information
        bloodGroup: z.enum(['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-', '']).optional(),
        personalEmail: z.string().trim().email({ message: 'Invalid email address' }).optional().or(z.literal('')),
